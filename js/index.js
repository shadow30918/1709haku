...
$(document).ready(function(){
  ...
  //index
	...
	$('.menu , .close, .link-yt, .link-blog, .link-tips, .link-info').click(function(e){
	  if($(window).width()<=960){
...
$('.top-words').delay(1300).fadeIn(800);
  ...
  $('.link-eye').click(function(e){
	...
	$('.link-tips').click(function(e){
		e.preventDefault();
		$('html,body').animate({
			scrollTop:$('.tips').offset().top - 56
		},800);
		/*$('.menu-bg').slideUp(800);
		$('.menu').css('display','inline-block');
		$('.close').css('display','none');*/
	...
	$('.back-btn').click(function(){
		...
		$('.info-words,.input,.agree,.info-btn,.info-note,.info-prod').fadeIn(300);
  ...
  function CityStoreListByEvent(){
	...
	function checkData(){
      ...
      if(c_no == '' || s_no == ''){
		...
		if(check_format == 1){
			$('.info-words,.input,.agree,.info-btn,.info-prod,.info-note').fadeOut(200);
			$('.done-title,.done-info,.done-check,.done-btn').fadeIn(300);
			$('.done-check .name-title').append('<span class="show-info">' + $('#name').val() + '</span>');
			$('.done-check .phone-title').append('<span class="show-info">' + $('#phone').val() + '</span>');
	...
	function SubmitForm(){
      ...
      /*
			if(re_ok == 1){
				...
				token = data.token;
				window.adGeek.conversion({callback: function () { window.location = "https://www.shiseido-event.com/2017melanoreduce/thank.html?c_no="+c_no+"&s_no="+s_no+"&t_no="+t_no+"&t_sn_no="+t_sn_no+'&token='+token; }});
			}*/
		});
	}
